 ...
 * DB, RLS and guard logic are real. redirect() throws NEXT_REDIRECT.
 */
import { afterAll, beforeAll, describe, expect, it, vi } from "vitest";

import { adminUrl } from "../../../test/db-url";
...
import { currentGroup, requireMembership, requireUser, SEED_GROUP_SLUG } from "./guards";
...
beforeAll(async () => {
  ...
  groupId = (await db.group.create({ data: { slug: SEED_GROUP_SLUG, name: "קבוצה" } })).id;
...
describe("currentGroup", () => {
  it("resolves THE seeded group deterministically, even with other groups present", async () => {
    ...
    expect(g.slug).toBe(SEED_GROUP_SLUG);
  });
});
