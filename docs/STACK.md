...
- next-auth **5.0.0-beta.31** + @auth/prisma-adapter **2.11.2** · tsx (seed runner). **No nodemailer** — unfixed high-sev advisories; custom email provider console-logs in dev, throws in prod.
- Not yet installed (join at their slice): TipTap, Tailwind, Playwright, sharp.
...
## Known danger zones
- **Search folding is decided in ONE place — `bm_normalize` in SQL.** Query text MUST be folded by calling `SELECT bm_normalize(...)`, never by a TypeScript re-implementation: if index and query ever fold differently, search returns zero hits with no error anywhere. Related decisions recorded here because SPEC §8 does not state them: title is weighted `A` and description `B` (future body text gets `C`); final letters are folded, which helps the TRIGRAM path only; `searchText` is folded and therefore NOT displayable — a snippet/highlight feature must `ts_headline` the raw columns.
- **Changing `bm_normalize` is a full table rewrite.** `CREATE OR REPLACE` does NOT recompute STORED generated columns (verified): old rows keep the old folding while queries use the new one, and search silently stops finding them. The only safe edit, in ONE migration: DROP the columns and their indexes, replace the function, re-ADD columns and indexes.
- **Generated columns + partial indexes are invisible to Prisma.** `searchVector`/`searchText` are `GENERATED ALWAYS … STORED`, and several uniques are PARTIAL (live rows only, ADR 0002) — Prisma can express neither. They are declared as `Unsupported()` / left undeclared with NOTE comments so `migrate` will not drop them, and **hand-authored migrations + `migrate deploy` are the workflow**; `migrate dev` must not be trusted to generate against these tables. One residual drift remains (`ALTER COLUMN … DROP DEFAULT` on the generated columns) — Postgres REFUSES it outright ("is a generated column"), so it fails loudly rather than corrupting.
- **Email delivery is an unmade deploy decision** (Resend-style API vs SMTP relay + SPF/DKIM/DMARC per SPEC §6). The magic-link provider throws in production until wired — deploy gate, owner's call.
